/**
 * MECHANISM TEMPLATE v2.7.0
 * ======================================
 * JANGAN edit file .gs form langsung untuk logic approval.
 * Edit file ini, lalu jalankan: python3 sync-mechanism.py
 *
 * Placeholder per form (diisi otomatis dari forms.config.json):
 *   {URL}        - Web App URL setelah deploy
 *   {SHEETNAME}  - Nama tab response spreadsheet
 *   {UID_PREFIX} - Prefix UID (OL-, MSI-, dll)
 */

function App() {
  this.form = FormApp.getActiveForm()
  this.formUrl = this.form.getPublishedUrl()
  this.url = "{{URL}}" // IMPORTANT - copy the web app url after deploy
  this.title = this.form.getTitle()
  this.desription = this.form.getDescription()
  this.sheetname = "{{SHEETNAME}}" // DO NOT change - the default google form responses sheet name
  this.flowHeader = "Divisi" // IMPORTANT - key field for your flows
  this.uidHeader = "_uid"
  this.uidPrefix = "{{UID_PREFIX}}"
  this.uidLength = 5
